<#
.SYNOPSIS
    Used to disable multiple accounts, strip all memberships and move to the appropriate OU.
.DESCRIPTION
    This script is designed to disable an account, remove all groups and move the account to the disabled users OU. Note this is only to be used on accounts that are not tied to an active Datacentre record (Eg, Admin, External and duplicate accounts)
.Outputs
	An extract will be created containing the users groups and general account info at the time of disablement.
.NOTES
    Generated On: 03/06/2021
    Author: Matthew Heuer
#>
Import-Module ActiveDirectory

$SAMS = Get-Content 'C:\temp\Imports\SecurityDisable.txt'
$USERS = @()
...
$UserCheck = Read-Host 'Do you want to continue? (Y\N)'
if ($UserCheck -like 'y') {
    #Ignore this
} 
...
Write-Host "Accounts successfully disabled!" -ForegroundColor Green